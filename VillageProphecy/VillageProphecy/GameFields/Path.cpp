...
* pathTexture: pointer to the texture for the path 
*/
Path::Path(Areas area, Direction dir, Vector2f position, Texture *pathTexture) : leadsTo(area), direction(dir){
	//Loads path texture
...
Vector2u Path::getSize(){
	return pathSprite.getTexture()->getSize();
}
